...
module.exports = {
    name: "dccezalı",
    command: ["dccezali","dc-cezali","dc-cezalı","dcc"],
  ...
  komutClient: async function (client, message, args) {
    ...
    let dcCezalı = await Punitives.findOne({member: cartelcim.id, type: "DC Cezalandırma" })
    const sebeps = [
    ...
    let jailOptions = new MessageActionRow().addComponents(
        ...
        .setCustomId(`sebep`)
        .setPlaceholder(`${cartelcim.user.username} için ceza sebebi belirt.`)
        .addOptions([
...
Cezalıya atılma tarihi: ${dcCezalı.Date}
