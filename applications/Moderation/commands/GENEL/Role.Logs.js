...
module.exports = {
    name: "rollog",
    command: ["rollog","rolgeçmişi"],
    aliases: "rollog @cartel/ID",
  ...
  komutClient: async function (client, message, args) {
    ...
    Users.findOne({_id: cartelcim.id }, async (err, res) => {
      if (!res) return message.channel.send({embeds: [new richEmbed().üstBaşlık(cartelcim.user.username, cartelcim.user.avatarURL({dynamic: true})).açıklama(`${cartelcim} kişisinin rol bilgisi bulunamadı.`)]})
      if(!res.Roles) return message.channel.send({embeds: [new richEmbed().üstBaşlık(cartelcim.user.username, cartelcim.user.avatarURL({dynamic: true})).açıklama(`${cartelcim} kişisinin rol bilgisi bulunamadı.`)]})
      let pages = res.Roles.sort((a, b) => b.tarih - a.tarih).chunk(10);
      var currentPage = 1
      if (!pages && !pages.length || !pages[currentPage - 1]) return message.reply({embeds: [new richEmbed().üstBaşlık(cartelcim.user.username, cartelcim.user.avatarURL({dynamic: true})).açıklama(`${cartelcim} kişisinin rol bilgisi bulunamadı.`)]})
      ...
      await curPage.edit({embeds: [embed.açıklama(`${pages[currentPage - 1].map((x, index) => `\` ${index+1} \` ${Array.isArray(x.rol) ? x.rol.map(x => message.guild.roles.cache.get(x)).join(", ") : message.guild.roles.cache.get(x.rol)} <t:${String(x.tarih).slice(0, 10)}:R> [**${x.state == "Ekleme" ? "EKLEME" : "KALDIRMA"}**] (<@${x.mod ? x.mod : "Bilinmeyen Yetkili"}>)`).join("\n")}`)]}).catch(err => {})
      ...
      collector.on("collect", async (i) => {
        ...
        await i.deferUpdate();
        await curPage.edit({
          embeds: [embed.altBaşlık(`${ayarlar.serverName ? ayarlar.serverName : message.guild.name} • ${currentPage} / ${pages.length} `, message.guild.iconURL({dynamic: true})).açıklama(`${pages[currentPage - 1].map((x, index) => `\` ${index+1} \` ${Array.isArray(x.rol) ? x.rol.map(x => message.guild.roles.cache.get(x)).join(", ") : message.guild.roles.cache.get(x.rol)} <t:${String(x.tarih).slice(0, 10)}:R> [**${x.state == "Ekleme" ? "EKLEME" : "KALDIRMA"}**] (<@${x.mod ? x.mod : "Bilinmeyen Yetkili"}>)`).join("\n")}`)]
        }).catch(err => {});
        collector.resetTimer();
      });
      collector.on("end", () => {
        if(curPage) curPage.edit({
          embeds: [embed.altBaşlık(`${ayarlar.serverName ? ayarlar.serverName : message.guild.name}`, message.guild.iconURL({dynamic: true})).açıklama(`${cartelcim} kişisinin toplamda \`${res.Roles.length || 0}\` adet rol bilgisi mevcut.`)],
          components: [],
        }).catch(err => {});
      })
    })
