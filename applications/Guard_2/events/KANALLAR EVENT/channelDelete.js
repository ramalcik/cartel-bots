...
module.exports = async (channel) => {
  ...
  if(Data && !Data.channelGuard) return;
    ...
    let embed = new richEmbed().üstBaşlık(entry.executor.username, entry.executor.avatarURL({ dynamic: true})).başlık("Sunucuda Kanal Silindi!")
    ...
    embed.açıklama(`${entry.executor} (\`${entry.executor.id}\`) tarafından \`#${channel.name}\` isimli kanal silindi ve geri oluşturularak yapan kişi yasaklandı.`);
