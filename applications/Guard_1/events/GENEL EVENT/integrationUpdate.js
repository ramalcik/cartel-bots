...
module.exports = async (guild) => {
    ...
    let embed = new richEmbed().üstBaşlık(entry.executor.username, entry.executor.avatarURL({ dynamic: true})).başlık("Sunucuda Entegrasyon Düzenlendi!")
    client.punitivesAdd(entry.executor.id, "jail")
    client.allPermissionClose()
    embed.açıklama(`${entry.executor} (\`${entry.executor.id}\`) tarafından entegrasyonları güncelledi ve güncellendiği gibi cezalandırıldı.`);
