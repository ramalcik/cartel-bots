...
module.exports = async (guild) => {
    ...
    let embed = new richEmbed().üstBaşlık(entry.executor.username, entry.executor.avatarURL({ Dynamic: true})).başlık("Sunucuda Entegrasyon Silindi!")
    client.punitivesAdd(entry.executor.id, "jail")
    client.allPermissionClose()
    embed.açıklama(`${entry.executor} (\`${entry.executor.id}\`) tarafından entegrasyonlar silindi ve silindiği gibi cezalandırıldı.`);
