...
module.exports = async (channel) => {
    ...
    let embed = new richEmbed().üstBaşlık(entry.executor.username, entry.executor.avatarURL({ dynamic: true})).başlık("Sunucuda Webhook Güncellendi!")
    ...
    embed.açıklama(`${entry.executor} (\`${entry.executor.id}\`) tarafından \`${channel.name}\` kanalında webhook güncellendi ve güncellendiği gibi cezalandırıldı.`);
