            ...
            if(difference > DIFF) {
                ...
                usersMap.set(message.author.id, userData)
                    ...
                    if(parseInt(msgCount) === LIMIT) {
                        ...
                        usersMap.delete(message.author.id);
                        await message.reply({content: `${message.guild.emojiyiBul(emojiler.chatSusturuldu)} ${message.member} Sohbet kanallarında fazla hızlı mesaj gönderdiğiniz için \` 1 Dakika \` süresince susturuldunuz.`}).then(x => setTimeout(() => {
                            x.delete().catch(err => {})
                        }, 7500)).catch(err => {})
...
client.on("messageDelete", async (message, channel) => {
    if(message.webhookId || message.author.bot || message.channel.type === "dm") return;
      ...
      let silinenMesaj = message.guild.kanalıBul("mesaj-log")
      if(!silinenMesaj) return;
      const embed = new richEmbed()
      .üstBaşlık(`Mesaj Silindi`, message.author.avatarURL())
      .açıklama(`${message.author.tag} kişisi bir mesaj sildi.`)
      .sütun("Kanal Adı", `${message.channel.name}`, true)
      .sütun("Silinen Mesaj", `${message.content ? `${message.content.length > 0 ? message.content : "Bir mesaj bulunamadı!"}` : "Bir mesaj bulunamadı!"}`)
      .sütun("Silinen Resimler", `${message.attachments.size > 0 ? message.attachments.filter(({ proxyURL }) => /\.(gif|jpe?g|png|webp)$/i.test(proxyURL)).map(({ proxyURL }) => proxyURL) : "Silinen resim bulunamadı."}`)
...
client.on("messageUpdate", async (oldMessage, newMessage) => {
    if(newMessage.webhookId || newMessage.author.bot || newMessage.channel.type === "dm") return;
      let guncellenenMesaj = newMessage.guild.kanalıBul("mesaj-log")
      if(!guncellenenMesaj) return;
      if (oldMessage.content == newMessage.content) return;
      let embed = new richEmbed()
      .üstBaşlık(`Mesaj Düzenlendi`, newMessage.author.avatarURL())
      .açıklama(`${newMessage.author} kişisi bir mesaj düzenledi`)
      .sütun("Eski Mesaj", `${oldMessage.content}`, true)
      .sütun("Yeni Mesaj", `${newMessage.content}`, true)
      .sütun("Kanal Adı", `${newMessage.channel.name}`, true)
...
async function sendChat(message, content) {
    if (getLimit.get(message.author.id) == 3) {
        ...
        await message.reply({content: `${message.guild.emojiyiBul(emojiler.chatSusturuldu)} ${message.member} Sohbet kanallarında ki kurallara uyum sağlanmadığı için \` 10 Dakika \` süresince susturuldunuz.`}).then(x => setTimeout(() => {
            x.delete().catch(err => {})
        }, 7500)).catch(err => {})
        ...
        message.delete().catch(err => {})
        let embed = new richEmbed()
        message.channel.send({content: `${message.member}`, embeds: [embed.açıklama(`Merhaba ${message.member}!
