...
module.exports = async (oldMember, newMember) => {
    ...
    let embed = new richEmbed().üstBaşlık(newMember.user.username, newMember.user.avatarURL({ dynamic: true}))
    ...
    if(!entry || !entry.executor || entry.createdTimestamp <= Date.now() - 5000 || await client.checkMember(entry.executor.id, "member" ,"Sağ-Tık Rol/Ver Alma!")) return;
        if (permissionStaff.some(p => oldMember.permissions.has(p) && newMember.permissions.has(p))) {
                ...
                embed.açıklama(`${newMember} (__${newMember.id}__) kişisine ${entry.executor} (__${entry.executor.id}__) tarafından Sağtık Rol İşlemi Yapıldı! Veren kişi cezalandırıldı ve verilen kişiden rol geri alındı.`);
                let loged = newMember.guild.kanalıBul("cartelfx");
...
client.on("guildMemberNicknameUpdate", async (member, oldNickname, newNickname) => {
    ...
    if(entry && entry.executor && !entry.executor.bot) {
        if(await client.checkMember(entry.executor.id, "member" ,"Sağ-Tık İsim Değiştirme!")) {
            ...
            let Log = member.guild.kanalıBul("isim-log")
            if(Log) Log.send({embeds: [embed.açıklama(`${member} (__${member.id}__) kişisinin ismi \`${oldNickname == null ? member.user.username : oldNickname} => ${newNickname == null ? member.user.username : newNickname}\` olarak ${entry.executor} (__${entry.executor.id}__) tarafından **${tarihsel(Date.now())}** güncellendi.`)]})
        }
    }
    ...
    embed.açıklama(`${member} (__${member.id}__) kişisinin ismi \`${oldNickname == null ? member.user.username : oldNickname} => ${newNickname == null ? member.user.username : newNickname}\` olarak ${entry.executor} (__${entry.executor.id}__) tarafından izinsiz olarak güncellendi ve ${member} kişisinin ismi eski haline getirildi.`);
