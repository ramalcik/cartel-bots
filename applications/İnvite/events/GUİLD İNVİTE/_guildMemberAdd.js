...
module.exports = async (member) => {
    ...
    let entry = await member.guild.fetchAuditLogs({ type: 'BOT_ADD' }).then(audit => audit.entries.first());
        if (member.user.bot && entry) return channel.send({ content: `${member} isimli bot, **${entry.executor.tag}** tarafÄ±ndan sunucuya davet edildi.`})
    ...
    if (invite === null) {
      channel.send({ content: `${member.guild.emojiyiBul("sonraki")} ${member} kişisi sunucumuza katıldı, davet edeni bulamadım.`})
      ...
      return channel.send({ content: `${member.guild.emojiyiBul("sonraki")} ${member} kişisi sunucumuza **ÖZEL URL** davet ile katıldı.`})
    } else {
      ...
      if (Date.now()-member.user.createdTimestamp <= 1000*60*60*24*7) {
        ...
        const total = inviterData ? inviterData.total : 0;
        channel.send({ content: `${member.guild.emojiyiBul("sonraki")} ${member} kişisi sunucumuza ${invite.inviter} tarafından davet edildi, ve bu kişisinin ${total} daveti oldu.`})
      } else {
        ...
        channel.send({ content: `${member.guild.emojiyiBul("sonraki")} ${member} kişisi sunucumuza ${invite.inviter} tarafından davet edildi, ve bu kişinin ${total} daveti oldu.`})
      }
    }
}
