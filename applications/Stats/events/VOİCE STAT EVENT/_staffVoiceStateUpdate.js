    ...
    setInterval( async () => {
        ...
        channels.forEach(channel => {
            ...
            members.forEach(async (member) => {
                if(member.roles.cache.has(roller.cezalıRolü) || member.roles.cache.has(roller.şüpheliRolü) || member.roles.cache.has(roller.yasaklıTagRolü) || member.roles.cache.has(roller.underworldRolü) || (roller.kayıtsızRolleri && roller.kayıtsızRolleri.some(rol => member.roles.cache.has(rol)))) return;
                ...
                if(ayarlar.fullPuanKategoriler.includes(channel.parentId)) {
