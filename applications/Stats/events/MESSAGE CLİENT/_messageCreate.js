...
client.on("messageCreate", async (message) => {
    ...
    Stats.findOne({ guildID: message.guild.id, userID: message.author.id }, async (err, data) => {
        ...
        if (!data) {
            ...
            if(_statSystem.sistemcik && _statSystem.staffs.some(x => message.member.roles.cache.has(x.rol))) data.upstaffChatStats.set(kanalID, Number(lastStaffData)+1);
            data.save();
            dayStatsUpdate(message.author.id, message.channel.id)
    };
  }); 
...
module.exports = async (message) => {
    ...
    if(message.member.roles.cache.has(roller.cezalıRolü) || message.member.roles.cache.has(roller.şüpheliRolü) || message.member.roles.cache.has(roller.yasaklıTagRolü) || (roller.kayıtsızRolleri && roller.kayıtsızRolleri.some(rol => message.member.roles.cache.has(rol)))) return;
...
Sohbet seviyeniz yükseldi ve ödülleri aldınız.`, files: [
               attach
            ]}).then(async (msg) => {
                await client.Economy.bakiyeGüncelle(message.author.id, Number(2500*(_getStat.messageLevel+1)), "add", 1)
                await client.Economy.bakiyeGüncelle(message.author.id, Number(1), "add", 0)  
