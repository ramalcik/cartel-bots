...
module.exports = async role => {
  ...
  if(Data && !Data.roleGuard) return;
    let embed = new richEmbed().üstBaşlık(role.member.user.username, role.member.user.avatarURL({ dynamic: true})).başlık("Sunucuda Rol Silindi!")
    ...
    if(arr.includes(role.id)) {
        embed.üstBaşlık(member.user.username, member.user.avatarURL({ dynamic: true})).başlık("Sunucuda Özel Bir Rol Silindi!")
        ...
        embed.açıklama(`${entry.executor} (\`${entry.executor.id}\`) tarafından **@${role.name}** (\`${role.id}\`) isimli özel rol silindi, silen kişi sunucudan uzaklaştırılarak silinen rol tekrardan kurulmaya başlandı.
    
**Rol İsmi**: \` @${newRole.name} \` [${newRole}]
**Dağıtılacak Kişi Bilgisi**: \` ${roleData.members.length || 0} kişi \`
**Tahmini Dağıtım Süresi**: \` ${roleData ? roleData.members.length > 0 ? (roleData.members.length>1000 ? parseInt((roleData.members.length*(250/1000)) / 60)+" dakika" : parseInt(roleData.members.length*(250/1000))+" saniye") : "Belirlenemedi." : "Belirlenemedi." } \``).altBaşlık("yukarda belirtilen rol bilgisi dahilinde tekrardan kurulum sağlanmıştır ve dağıtımı başlamıştır.");
    ...
    embed.açıklama(`${entry.executor} (\`${entry.executor.id}\`) tarafından **@${role.name}** (\`${role.id}\`) isimli rol silindi, silen kişi sunucudan uzaklaştırılarak silinen rol tekrardan kurulmaya başlandı.
