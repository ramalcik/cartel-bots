...
module.exports = {
  ...
  komutClient: async function (client, message, args) {
    ...
    if (!args[0] || isNaN(args[0])) return message.channel.send({embeds: [ new richEmbed().açıklama(`Bir ses kanalı id'si belirt ve tekrar dene.`)]})
    VoiceChannels.findOne({ channelID: args[0] }, async (err, data) => {
      ...
      await message.channel.send({ embeds: [embed.açıklama(`**${newChannel.name}** isimli ses kanalının, \`${tarihsel(Date.now())}\` tarihli ses kanalı kurulmaya başladı.`)]})
      const newOverwrite = [];
