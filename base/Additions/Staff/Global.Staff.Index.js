...
const yarraağ = require("../../../database/Schemas/Plugins/Client.GörevAlanlar");

class Staff {

    static async showPoint(id) {
        var reload = require('require-reload')(require);
        _statSystem = global._statSystem = uPConf=  reload('./Sources/Global.Staff.Settings.js');
    ...
    static async removePoint(id, pnt, tip) {
        var reload = require('require-reload')(require);
        _statSystem = global._statSystem = uPConf=  reload('./Sources/Global.Staff.Settings.js');
        if(!uPConf.sistemcik) return client.logger.log(`UpStaff Sistemi Kapalı fakat kullanılmaya çalışılıyor.`, "ups");
    ...
    static async addPoint(id, pnt, tip, categoryID) {
        const kontrolvscartel = yarraağ.findOne({ _id: id})
        var reload = require('require-reload')(require);
        _statSystem = global._statSystem = uPConf=  reload('./Sources/Global.Staff.Settings.js');
        if(!uPConf.sistemcik) return client.logger.log(`UpStaff Sistemi Kapalı fakat kullanılmaya çalışılıyor.`, "ups");
        const guild = client.guilds.cache.get(sistem.SUNUCU.GUILD) 
        ...
        if(!uP && görevGetir) {
            if(pnt < 0) return;
            return await Upstaffs.updateOne({ _id: cartelcim.id }, { $set: { "staffNo": 1, "staffExNo": 0, "Point": pnt, "ToplamPuan": pnt, "yetkiVerilmeTarihi": Date.now(), "Rolde": Date.now() } }, {upsert: true}); 
        }
        if(!uP) {
            if(!uPConf.staffs.some(x => cartelcim.roles.cache.has(x.rol))) return;
            if(pnt < 0) return;
            if(cartelcim.roles.cache.has(ayarlar.sonYetki)) return;
            await Upstaffs.updateOne({ _id: cartelcim.id }, { $set: { "staffNo": 1, "staffExNo": 0, "Point": pnt, "ToplamPuan": pnt, "yetkiVerilmeTarihi": Date.now(), "Rolde": Date.now() } }, {upsert: true}); 
        ...
        if((uP.Yönetim || görevGetir) && uP.Mission && !uP.Mission.Completed) {
            ...
            let görevLog = cartelcim.guild.kanalıBul("görev-bilgi");
            if(MissionData) {
...
if (kontrolvscartel && kontrolvscartel.Görevi === "Davet etme" && Date.now() < MissionData.Time) {
                if( tip == "Invite" && MissionData.Invite >= 1 && !uP.Mission.CompletedInvite) {
                    ...
                    if(uP.Mission.Invite + 1 >= MissionData.Invite) {
                        if(görevLog) görevLog.send({embeds: [logEmbed.açıklama(`${cartelcim} isimli görev sahibi, **Davet** görevini <t:${String(Date.now()).slice(0, 10)}:R> tamamlayarak \`${MissionData.Reward ? MissionData.Reward  : uPConf.points.tasks}  Görev Puanı\` kazandı.`)]})
                        ...
                        await client.Economy.bakiyeGüncelle(cartelcim.id, Number(MissionData.Reward), "add", 1) 

                    }
                }
            }
            if (kontrolvscartel && kontrolvscartel.Görevi === "Taglı çekme" && Date.now() < MissionData.Time) {
                if(tip == "Taglı" && MissionData.Tagged >= 1 && !uP.Mission.CompletedTagged) {
                    ...
                    if(uP.Mission.Tagged + 1 >= MissionData.Tagged) {
                        if(görevLog) görevLog.send({embeds: [logEmbed.açıklama(`${cartelcim} isimli görev sahibi, **Taglı** görevini <t:${String(Date.now()).slice(0, 10)}:R> tamamlayarak \`${MissionData.Reward ? MissionData.Reward  : uPConf.points.tasks}  Görev Puanı\` kazandı.`)]})
                        await Users.updateOne({_id: cartelcim.id}, {$inc: {"Coin": MissionData.Reward ? MissionData.Reward  : uPConf.points.tasks}}, {upsert: true})
            ...
            if (kontrolvscartel && kontrolvscartel.Görevi === "Kayıt etme" && Date.now() < MissionData.Time) {
                if(tip == "Kayıt" && MissionData.Register >= 1 && !uP.Mission.CompletedRegister) {
                    ...
                    if(uP.Mission.Register + 1 >= MissionData.Register) {
                        if(görevLog) görevLog.send({embeds: [logEmbed.açıklama(`${cartelcim} isimli görev sahibi, **Kayıt** görevini <t:${String(Date.now()).slice(0, 10)}:R> tamamlayarak \`${MissionData.Reward ? MissionData.Reward  : uPConf.points.tasks}  Görev Puanı\` kazandı.`)]})
                        await Users.updateOne({_id: cartelcim.id}, {$inc: {"Coin": MissionData.Reward ? MissionData.Reward  : uPConf.points.tasks}}, {upsert: true})
            ...
            if (kontrolvscartel && kontrolvscartel.Görevi === "Yetkili" && Date.now() < MissionData.Time) {
                if(tip == "Yetkili" && MissionData.Staff >= 1 && !uP.Mission.CompletedStaff) {
                    ...
                    if(uP.Mission.Staff + 1 >= MissionData.Staff) {
                        if(görevLog) görevLog.send({embeds: [logEmbed.açıklama(`${cartelcim} isimli görev sahibi, **Yetkili** görevini <t:${String(Date.now()).slice(0, 10)}:R> tamamlayarak \`${MissionData.Reward ? MissionData.Reward  : uPConf.points.tasks}  Görev Puanı\` kazandı.`)]})
                        await Users.updateOne({_id: cartelcim.id}, {$inc: {"Coin": MissionData.Reward ? MissionData.Reward  : uPConf.points.tasks}}, {upsert: true})
                ...
                if(tip == "Sorumluluk" && MissionData.Sorumluluk >= 1 && !uP.Mission.CompletedSorumluluk) {
                    ...
                    if(uP.Mission.Sorumluluk >= MissionData.Sorumluluk) {
                        if(görevLog) görevLog.send({embeds: [logEmbed.açıklama(`${cartelcim} isimli görev sahibi, **Liderlik/Sorumluluk** görevini <t:${String(Date.now()).slice(0, 10)}:R> tamamlayarak \`${MissionData.Reward ? MissionData.Reward  : uPConf.points.tasks}  Görev Puanı\` kazandı.`)]})
        ...
        if (uP && uPConf.staffs.some(x => uP.Point >= yeniPuan)) {
          ...
          await Users.updateOne({ _id: cartelcim.id }, { $push: { "StaffLogs": {
              ...
              Role: yeniYetki ? yeniYetki.rol : roller.ilkYetki,
              Author: cartelcim.id
          }}}, { upsert: true }) 
          
          let embed = new richEmbed()
          let yetkiAtladınKanal = guild.kanalıBul(kanallar.TerfiLog) || guild.kanalıBul("terfi-log") || guild.kanalıBul("yetki-atladın")
          if(yetkiAtladınKanal)  yetkiAtladınKanal.send({embeds: [embed.açıklama(`${cartelcim.toString()} kişisi gereken puana ulaştı ve ${yeniYetki.exrol ? `<@&${yeniYetki.rol}>, `+ yeniYetki.exrol.map(x => cartelcim.guild.roles.cache.get(x)).join(", ") : Array.isArray(yeniYetki.rol) ? yeniYetki.rol.map(x => `<@&${x}>`).join(", ") : `<@&${yeniYetki.rol}>`} isimli yetki rol(leri) verildi!`)]});
          ...
          await client.Economy.bakiyeGüncelle(cartelcim.id, Number(yeniPuan), "add", 1)
	} 
