...
module.exports = async (options) => {
	...
	async function gameOver(m) {
		...
		const editEmbed = new richEmbed().renk("RANDOM")
		let embedicerik = options.embed.description.replace('{{score}}', score).replace('{{kazanilan}}', Number(score * 10 * 5))
		if(score > 0) await client.Economy.bakiyeGüncelle(options.message.member.id, Number(score * 10 * 5), "add", 1)
		editEmbed.açıklama(embedicerik);
		m.edit({
			embeds: [editEmbed],
			components: [
