...
.setEmoji(options.message.guild.emojiyiBul(emojiler.no_cartel));
            ...
            await client.Economy.bakiyeGüncelle(options.opponent.id, options.bahis, "remove", 1)
            await client.Economy.bakiyeGüncelle(options.message.author.id, options.bahis, "remove", 1)
			Collector.stop();
			...
			const emd = new richEmbed()
				...
				.açıklama(options.embed.description)
            ...
            await client.Economy.bakiyeGüncelle(options.opponent.id, options.bahis, "add", 1)
            await client.Economy.bakiyeGüncelle(options.message.author.id, options.bahis, "add", 1)
							} else if (
...
await client.Economy.bakiyeGüncelle(options.opponent.id, options.ödül, "add", 1)
...
await client.Economy.bakiyeGüncelle(options.message.author.id, options.ödül, "add", 1)
;
							}
			...
			collector.on('end', async (collected, reason) => {
				if (reason === 'time') {
					...
					const _embed = new richEmbed()
						...
						.açıklama(options.timeEndMessage)
