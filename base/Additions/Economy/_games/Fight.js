...
module.exports = async (options) => {
	...
	if (data.has(options.message.author.id) || data.has(options.opponent.id)) {
		return options.message.react(options.message.guild.emojiyiBul(emojiler.no_cartel)).catch(err => {});
	}
	...
	if (oppenent.bot || oppenent.id === challenger.id) return options.message.react(options.message.guild.emojiyiBul(emojiler.no_cartel)).catch(err => {});
	let acceptbutton = new Discord.MessageButton()
		.setStyle('SECONDARY')
		.setLabel(options.buttons.accept)
		.setCustomId("accept")
.setEmoji(options.message.guild.emojiyiBul(emojiler.onay_cartel));;
	let denybutton = new Discord.MessageButton()
		.setStyle('DANGER')
		.setLabel(options.buttons.deny)
		.setCustomId("deny")
.setEmoji(options.message.guild.emojiyiBul(emojiler.no_cartel));;
...
.setEmoji(options.message.guild.emojiyiBul(emojiler.no_cartel));;
			...
			Collector.stop();
            await client.Economy.bakiyeGüncelle(challenger.id, options.bahis, "remove", 1)
            await client.Economy.bakiyeGüncelle(oppenent.id, options.bahis, "remove", 1)
...
Savaş devam ediyor...`,
							...
							if (gameData[player].health < 50) {
								...
								const __embed = new richEmbed()
									...
									.renk(options.embed.color);
                                    await client.Economy.bakiyeGüncelle(gameData[player].member.id, options.bahis, "add", 1)
								question.edit({
									embeds: [__embed],
									components: [],
								});
						...
						if (checkHealth(player)) {
							...
							const __embed = new richEmbed()
								...
								.renk(options.embed.color);
                                await client.Economy.bakiyeGüncelle(gameData[tempPlayer].member.id, options.ödül, "add", 1)
...
.setEmoji(options.message.guild.emojiyiBul(emojiler.no_cartel));;
			...
			const _embed = new richEmbed()
				...
				.açıklama(options.endMessage.replace('{{opponent}}', oppenent.id))
