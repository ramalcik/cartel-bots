...
GuildMember.prototype.Register = async function (name, gender = undefined, registrant = undefined) {
    ...
    let seskanal = registrant.guild.channels.cache.filter(x => x.parentId == kanallar.publicKategorisi && x.type == "GUILD_VOICE" && x.id != kanallar.sleepRoom && (ayarlar.musicRooms && ayarlar.musicRooms.some(kanal => x.id != kanal))).random()
...
Sana İyi Sohbetler Dilerim! :tada:`}).catch(err => {})
    ...
    if(kanallar.chatKanalı) {
            ...
            if(ayarlar.type) {
                if(kanal) {
                    if(this.user.username.includes(ayarlar.tag)) kanal.send(`Aramıza yeni biri katıldı ${this} ona merhaba diyelim! ${this.guild.emojiyiBul("hareketliayicik")}`).then(x => {setTimeout(() => { x.delete() }, 15250)})
                    if(!this.user.username.includes(ayarlar.tag)) kanal.send(`Aramıza yeni biri katıldı ${this} ona merhaba diyelim! ${this.guild.emojiyiBul("hareketliayicik")}`).then(x => {setTimeout(() => { x.delete() }, 20000)}) 
                }
            } else {
                if(kanal) kanal.send(`Aramıza yeni biri katıldı ${this} ona merhaba diyelim! ${this.guild.emojiyiBul("hareketliayicik")}`).then(x => {setTimeout(() => { x.delete() }, 15250)})
            }
        }
    // Fiziksel Kayıt Mantığımız
    ...
    let kayıtLog = this.guild.kanalıBul("kayıt-log")
    if(kayıtLog) kayıtLog.send({embeds: [new richEmbed().üstBaşlık(this.user.username, this.user.avatarURL({ dynamic: true})).açıklama(`${this} kişisi ${registrant} tarafından  ${tarihsel(Date.now())} tarihinde **${gender}** olarak sunucuya kaydedildi.`)]})
...
GuildMember.prototype.removeStaff = async function(lastRole, manuel) {
    ...
    if(userData && userData.Staff && userData.yetkiVeren) {
        let Yetkili = await Users.findOne({_id: userData.yetkiVeren })
          if(Yetkili && Yetkili.Staffs) {
              const LanYarramBakSikildiler = this.guild.members.cache.get(userData.yetkiVeren)
              if(LanYarramBakSikildiler && !manuel) LanYarramBakSikildiler.send({content: `
${this} (**${this.user.username}**)
ID: ${this.id}`, embeds: [new richEmbed()
                .açıklama(`Merhaba ${LanYarramBakSikildiler} sunucumuza ${this} kullanıcısını yetkili olarak başlatmışsınız fakat <t:${String(Date.now()).slice(0, 10)}:R> yetkiyi bıraktı.`)
             ]}).catch(err => { })
              const findUser = Yetkili.Staffs.find(cartel => cartel.id == this.id);
              await Users.updateOne({ _id: userData.yetkiVeren }, { $pull: { "Staffs": findUser } }, { upsert: true })
              client.Upstaffs.removePoint(userData.yetkiVeren, -_statSystem.points.staff, "Yetkili")
          }
    }
    if(userData && userData.Staff) {
        await Users.updateOne({ _id: this.id }, { $set: { "Staff": false, "yetkiVeren": new String() } }, { upsert: true })
	...
	if(!lastRole) lastRole = this.roles.cache
        if(!manuel) {
            let altYetki = this.guild.roles.cache.get(roller.ilkYetki)
            let arr = []
            ...
            let yetkiyiBırakan = this.guild.kanalıBul("yetki-bırakan")
            let embeds = new richEmbed()
             .açıklama(`${this} kişisi yetkisini ${tarihsel(Date.now())} tarihinde bıraktı.`);
             if(Data) {
                let sorumlusu = this.guild.members.cache.get(userData.yetkiVeren)
                embeds.sütun(`Detaylar`,
                    `
Yetki veren: ${userData.yetkiVeren ? sorumlusu ? `${sorumlusu} (\`${sorumlusu.displayName}\`)` : `<@${userData.yetkiVeren}> (\`Sunucudan Ayrılmış!\`)`  : `\` Veri Bulunamadı! \`` }
Yetkide Kalma Süresi: \` ${Data.yetkiVerilmeTarihi ? sureCevir(Date.now() - Date.parse(Data.yetkiVerilmeTarihi)) : `Veri Bulunamadı!`} \` 
Yetki tarihi: ${Data.yetkiVerilmeTarihi ? `${tarihsel(Data.yetkiVerilmeTarihi)}` : `\` Veri Bulunamadı! \``}
...
${[...roller.altYönetimRolleri, ...roller.yönetimRolleri, ...roller.üstYönetimRolleri].filter(x => this.guild.roles.cache.get(x)).map(x => this.guild.roles.cache.get(x))}`, embeds: [
    ...
    collector.on('collect', async (i) => {
        ...
        if(kapan) {
            ...
            this.send({embeds: [new richEmbed(). setDescription(`Merhaba! ${this}
Sunucumuz da yetkiyi bıraktığınız için tüm rollerin alındı.
 ${kapan} kişisine yazarak rolünüze geri dönebilirsin.`)]}).then(a => {
                i.reply({embeds: [new richEmbed().açıklama(`${this} kişisine yetkiyi bıraktığı için özel mesaj üzerinden mesaj ile bildirildi.`)]})
            }).catch(async (err) => {
                removeTagLog.send(`Merhaba! ${kapan},
sorumluluk aldığınız kişinin özel mesajları kapalı olduğu için mesaj gönderilemedi.`);
...
GuildMember.prototype.removeTagged = async function () {
    let userData = await Users.findOne({_id: this.id})
    if(userData && userData.Tagged && userData.tagCağıran) {
        let Yetkili = await Users.findOne({_id: userData.tagCağıran }) 
           if(Yetkili && Yetkili.Taggeds) {
                const koşKoşBabanıSikiyorlar = this.guild.members.cache.get(userData.tagCağıran)
                const findUser = Yetkili.Taggeds.find(cartel => cartel.id == this.id);
                await Users.updateOne({ _id: userData.tagCağıran }, { $pull: { "Taggeds": findUser } }, { upsert: true })
                client.Upstaffs.removePoint(userData.tagCağıran, -_statSystem.points.tagged, "Taglı")
            } 
          await Users.updateOne({ _id: this.id }, { $set: { "Tagged": false, "tagCağıran": new String() } }, { upsert: true })
        }
}
    ...
    if(userData && userData.Name && userData.Registrant) {
        let Yetkili = await Users.findOne({_id: userData.Registrant }) 
          if(Yetkili && Yetkili.Records) {
              ...
              if(koşAnanıSikiyorlar) koşAnanıSikiyorlar.send({embeds: [new richEmbed().açıklama(`Kayıt ettiğiniz ${this} (${this.user.username}) (**Ceza Puanı**: \`${await this.cezaPuan()}\`) kişisi sunucuda **Kural Dışı** eylem sergiledi ve cezalandırıldı.
...
GuildMember.prototype._views = async function () {
    return new Promise(async (resolve, reject) => {
            ...
            resolve(`Başarıyla ${this.user.username} kişisinin görüntülemesi ${_cached.Views || 0} oldu.`)
    })
}
...
GuildMember.prototype.cezaPuan = async function(check, güm) {
        ...
        if(check) {
            let logKanalı = this.guild.kanalıBul("ceza-puan") || this.guild.kanalıBul("oto-ceza") 
            if((roller.Yetkiler && roller.Yetkiler.some(x => this.roles.cache.has(x))) && cezaPuanı >= 50) {
                ...
                if(yetkiliCezaPuanı == "BITTI") {
                    ...
                    await Users.updateOne({ _id: this.id }, { $push: { "StaffLogs": {
                        Date: Date.now(),
                        Process: "OTOMATİK ALINDI",
                        Role: this.roles.hoist ? this.roles.hoist.id : roller.ilkYetki,
                    ...
                    this.setRoles(roller.cezalıRolü ? roller.şüpheliRolü : "")
                    if(logKanalı) return logKanalı.send({embeds: [new richEmbed().açıklama(`${this} Yetkilisinin Yetkili Ceza Notu Düştüğü İçin Yaptığı Kural-Dışı Eylemlere Yaptırım Uygulanarak Yetkisi Çekildi Ve Şüpheli Konumuna Getirildi.`)]})
                }
                
                if(logKanalı) logKanalı.send({embeds: [new richEmbed().açıklama(`${this} Yetkilisinin Ceza Puanı Güncellendi. Fakat **50** Ceza Puanını Geçtiği İçin "Yetkili Ceza Notu" Sisteminden Düşüş Sağlanacak.
Düşüm Sonrası Yetkili Ceza Notu **0** Altına İndiğinde Otomatik Olarak Yetkisi Çekilecektir. Yetkili Ceza Notu: \`${yetkiliCezaPuanı}\` (Anlık Ceza Puanı: \`${cezaPuanı}\`)`)]})
            ...
            if((roller.Yetkiler && !roller.Yetkiler.some(x => this.roles.cache.has(x))) && cezaPuanı >= 50) {
                if(güm) this.setRoles(roller.cezalıRolü ? roller.şüpheliRolü : "")
                if(logKanalı) logKanalı.send({embeds: [new richEmbed().açıklama(`${this} kişisi **50** Ceza Puanını ${güm ? `Geçtiği İçin Otomatik Olarak Şüpheli Hesap Konumuna Gönderildi.` : `Geçti Fakat Son Atılan Cezası Şüpheli Konumundan Daha Üstün Olduğu İçin Gönderilemedi.`} Anlık Ceza Puanı: \`${cezaPuanı}\``)]})

            } else if((roller.Yetkiler && !roller.Yetkiler.some(x => this.roles.cache.has(x)))){
                if(logKanalı) logKanalı.send({embeds: [new richEmbed().açıklama(`${this} kişisinin Ceza Puanı Güncellendi. Anlık Ceza Puanı: \`${cezaPuanı}\``)]})
            }
        }
        return cezaPuanı;
}
...
GuildMember.prototype.Left = async function (lastRole) {
    ...
    if(userData && userData.Tagged && userData.tagCağıran) {
        let Yetkili = await Users.findOne({_id: userData.tagCağıran }) || {}
           if(Yetkili && Yetkili.Taggeds) {
                ...
                const findUser = Yetkili.Taggeds.find(cartel => cartel.id == this.id);
                await Users.updateOne({ _id: userData.tagCağıran }, { $pull: { "Taggeds": findUser } }, { upsert: true })
                await client.Upstaffs.removePoint(userData.yetkiVeren, -_statSystem.points.tagged, "Taglı")
            } 
          await Users.updateOne({ _id: this.id }, { $set: { "Tagged": false, "tagCağıran": new String() } }, { upsert: true })
        }
    ...
    if(userData && userData.Staff && userData.yetkiVeren) {
        let Yetkili = await Users.findOne({_id: userData.yetkiVeren }) || {}
          if(Yetkili && Yetkili.Staffs) {
              const BabanıSikeyim = this.guild.members.cache.get(userData.yetkiVeren)
              if(BabanıSikeyim) BabanıSikeyim.send({embeds: [new richEmbed().açıklama(`${BabanıSikeyim} Merhaba, ${this} (\`${this.id}\`) yetkiye aldığın yetkiyi sunucudan çıkarak <t:${String(Date.now()).slice(0, 10)}:R> bıraktı.`)]}).catch(err => { })
              const findUser = Yetkili.Staffs.find(cartel => cartel.id == this.id);
              await Users.updateOne({ _id: userData.yetkiVeren }, { $pull: { "Staffs": findUser } }, { upsert: true })
              client.Upstaffs.removePoint(userData.yetkiVeren, -_statSystem.points.staff, "Yetkili")
          } 
      ...
      if(userData && userData.Staff) {
        ...
        let yetkiyiBırakan = this.guild.kanalıBul("yetki-bırakan")
        if(yetkiyiBırakan) yetkiyiBırakan.send({content: `${this} [**\` ${this.id} \`**]
${[...roller.altYönetimRolleri, ...roller.yönetimRolleri, ...roller.üstYönetimRolleri].map(x => this.guild.roles.cache.get(x))}`,embeds: [new richEmbed().açıklama(`${this} isimli eski yetkili, yetkiyi sunucudan ayrılarak **<t:${String(Date.now()).slice(0, 10)}:R>** bıraktı.`).sütun("Son rolleri şunlardır", `${arr.map(x => this.guild.roles.cache.get(x)).join(", ")}`)]}) 
    }
}
    ...
    if(roles && roles.length > 0 && roles.some(b => this.roles.cache.has(b.role))) {
        ...
        for (let index = 0; index < roles.length; index++) {
            ...
            if(isimne.includes("lider")) {
                ...
                header = `Lider`
            } else if(isimne.includes("sorumlu") || isimne.includes("sorumluluğu") || isimne.includes("sorum") || isimne.inclides("Sorumlusu")) {
            ...
            .açıklama(`${this} kişisi **${point.toFixed(2)} Puan** Kazandı. Toplamda ${rol} ${uyarlama} **${Data.Responsibilitys[rol.id].toFixed(2)} Puan** Kazandı.
Bu İşlem <t:${String(Date.now()).slice(0, 10)}:R> / ${tarihsel(Date.now())} Tarihinde Yapıldı.
Yapılan İşlem Türü İse "**${detay.type}**" Olarak Tespit Edildi.`).altBaşlık(this.guild.name, this.guild.iconURL({dynamic: true}))
            if(option.user)  {
                let target = this.guild.members.cache.get(option.user)
                embed.sütun(`İşlem Yapılan Kullanıcı`, `${ target ? `${target} (\`${target.id}\`)` : `<@${option.user}> (\`Sunucudan Ayrılmış\`)`}`)
            }
            if(option.channel)  {
                let target = this.guild.channels.cache.get(option.channel) || this.guild.channels.cache.get(option.channel.id)
                embed.sütun(`İşlem Yapılan Kanal`, `${target ? `${target}` : `@deleted-channel`}`)
            }
            if(option.role)  {
                let target = this.guild.roles.cache.get(option.role)
                embed.sütun(`İşlem Yapılan Rol`, `${target ? `${target}` : `@deleted-roles`}`)
            }
...
GuildMember.prototype.addPunitives = async function(type, staff, reason = "Sebep belirtilmedi.", message, duration, muteFlood, rulesFlood, nofi = 1) {
    ...
    cezano = cezano == 0 ? 1 : cezano + 1;
        ...
        let logKanal = [
            {type: "Kalkmaz Yasaklama", channel: "açılmaz-ban-log"},
        ...
        if(_findLogChannel) {
            let findedChannel = message.guild.kanalıBul(_findLogChannel.channel)
            if(findedChannel) findedChannel.send({embeds: [new richEmbed().altBaşlık(`• Ceza Numarası: #${cezano}`,this.user.avatarURL({dynamic: true})).açıklama(`${this.toString()} kişisine <t:${String(Date.now()).slice(0, 10)}:F> tarihinde ${duration ? `${moment.duration(ms(duration)).format('Y [Yıl,] M [Ay,] d [Gün,] h [Saat,] m [Dakika]')} boyunca` : ``} \`${reason}\` nedeniyle ${staff.toString()} tarafından "${type}" türünde ceza işlemi uygulandı.`).üstBaşlık(this.user.username, this.user.avatarURL({ dynamic: true}) )]})
        }
        ...
        await this.send({embeds: [new richEmbed().üstBaşlık(this.user.username, this.user.avatarURL({ dynamic: true})).açıklama(`Sunucumuz da \`${staff.user.username}\` tarafından **${reason}** sebebi ile ${type == "Kalkmaz Yasaklama" ? 
                "kalkmaz yasaklamaya tabi tutuldun."
            : type == "Yasaklama" ? 
                "yasaklandın."
                : type == "Reklam" ? 
            ...
            : type == "Uyarılma" ?
            "uyarıldın."
            : `"${type}" türünde ceza-i işlem uygulandı.`} ${duration ? `Ceza bitiş tarihi \`${tarihsel(Date.now()+ms(duration))}\`. Eğer bu konu hakkında bir itirazın var ise üst yetkililerimize ulaşmaktan çekinme!` : "Eğer bu konu hakkında bir itirazın var ise üst yetkililerimize ulaşmaktan çekinme!"}`).altBaşlık(`(**Ceza Numarası**: \` #${cezano} \`)`)]}).catch(err => {
           
        })
        if(nofi == 0 && !muteFlood && !rulesFlood) await message.editReply({embeds: [new richEmbed().açıklama(`${this.toString()} kişiye **${reason}** sebebiyle ${duration ? `\`${moment.duration(ms(duration)).format('Y [Yıl,] M [Ay,] d [Gün,] h [Saat,] m [Dakika]')}\` boyunca` : ``} ${type == "Kalkmaz Yasaklama" ? 
        " kalkmaz yasaklamaya tabi tutuldun."
    : type == "Yasaklama" ? 
        "yasakladın."
    : type == "Cezalandırılma" ? 
    ...
    : type == "Uyarılma" ?
    "uyardın."
    : `"${type}" türünde ceza-i işlem uygulandı.`} ${message.guild.emojiyiBul(emojiler.onay_cartel)}`).altBaşlık(`${this.user.username} • Ceza Numarası: #${cezano} • ${type}`, this.user.avatarURL({dynamic: true}))], components: []})
        switch(type) {
            ...
            case "Cezalandırılma": {
                await this.voice.disconnect().catch(err => {})
                if(this && this.manageable) await this.setRoles(roller.cezalıRolü)
            ...
            case "Reklam": {
                await this.voice.disconnect().catch(err => {})
                if(this && this.manageable) await this.setRoles(roller.underworldRolü)
            ...
            case "Teyit Yasaklı": {
                await this.voice.disconnect().catch(err => {})
                if(this) await this.setRoles(roller.cezalıRolü)
